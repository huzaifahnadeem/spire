...
void parse_args(int ac, char **av, std::string &shadow_spinesd_ip_addr, int &shadow_spinesd_port);
...
std::string shadow_io_path = "./shadow_io";
...
int main(int ac, char **av) {
    ...
    parse_args(ac, av, shadow_spinesd_ip_addr, shadow_spinesd_port);
...
void parse_args(int ac, char **av, std::string &shadow_spinesd_ip_addr, int &shadow_spinesd_port) {
    shadow_spinesd_ip_addr = av[0]; // av element 0 because this program is supposed to be run as a child proc of proxy and when it runs from there it doesnt get the prog name at [0] like you normally do.
    shadow_spinesd_port = atoi(av[1]);
    shadow_io_path = av[2];
}
