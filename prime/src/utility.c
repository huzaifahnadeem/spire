...
void UTIL_Load_Addresses_From_Config(struct config *cfg)
{
	int32u unique_spines;
	int32u server, i;
	int replica_index = 1; // Prime indices start at 1

	Alarm(DEBUG, "Reading Spines addresses from config object\n");
	...
	// populate spines_daemon_address[] with unique spines daemon ips
	NET.num_spines_daemons = 0;
	// for each replica from 1 to replica_index
	for (server = 1; server < replica_index; server++)
	{
		// assume unique
		unique_spines = 1;
