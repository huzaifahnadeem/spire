...
int ipc_sock;
int ipc_sock_proxy;
itrc_data itrc_in; //, itrc_out; itrc_out moved to hmi-side proxy
itrc_data proxy_comm_data;
...
void itrc_init(int ac, char **av) 
{
    ...
    // setup IPC for proxy comm. thread:
    memset(&proxy_comm_data, 0, sizeof(itrc_data));
    sprintf(proxy_comm_data.prime_keys_dir, "%s", (char *)HMI_PRIME_KEYS);
    sprintf(proxy_comm_data.sm_keys_dir, "%s", (char *)HMI_SM_KEYS);
    sprintf(proxy_comm_data.ipc_local, "%s%d", (char *)HMI_IPC_ITRC, My_ID); // check
    sprintf(proxy_comm_data.ipc_remote, "%s%d", (char *)HMI_IPC_MAIN, My_ID); // check
    ipc_sock_proxy = IPC_DGram_Sock(proxy_comm_data.ipc_local);
}

void *proxy_client(void *arg)
{   
    // this function will use IPC to talk to the hmi-side proxy. the job is to receive updates from the proxy and send HMI commands (entered through the pvbrowser part of the code) to the proxy which forwards it to prime through spines.
    UNUSED(arg); // tmp
}
...
// TODO: remove this? it wouldnt work like this after the introducing hmi-side proxy so commented out for now.
// #ifdef USE_INETD
...
int main(int ac, char **av)
{
    PARAM p;
    int s;
    pthread_t tid; //, itid; // itid moved to hmi-side proxy
    pthread_t proxy_comm_thread;

    signal(SIGPIPE, SIG_IGN);
    modelInit();

    // itrc_init(ac, av);   // moved to hmi-side proxy
    // pthread_create(&itid, NULL, &ITRC_Client, (void *)&itrc_out);        // moved to hmi-side proxy
    pthread_create(&tid, NULL, &master_connection, NULL);
    void* arg_for_proxy_client = NULL;
    pthread_create(&proxy_comm_thread, NULL, &proxy_client, arg_for_proxy_client); // TODO: check 2nd and 4th param
