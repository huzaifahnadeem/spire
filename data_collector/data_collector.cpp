...
int main(int ac, char **av) {
    ...
    //     pthread_t mcast_listen_thread;
    //     pthread_create(&mcast_listen_thread, NULL, &mcast_listen, NULL);
    ...
    *  this often */
    ...
    //pthread_join(mcast_listen_thread, NULL);
    return EXIT_SUCCESS;
}
...
void* mcast_listen(void* fn_arg) {
    E_handle_events(); // signal libspread events handler to start handling all the events for which fds were set by the objects above (which in this case was only for mcast)
}
